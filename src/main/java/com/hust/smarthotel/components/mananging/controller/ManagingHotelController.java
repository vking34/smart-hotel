...
import static com.hust.smarthotel.generic.response.ErrorResponses.MANAGING_INVALID_TOKEN;
...
public class ManagingHotelController {
    ...
    private static final ResponseEntity<ManagingResponse> INVALID_TOKEN = new ResponseEntity<>(MANAGING_INVALID_TOKEN, HttpStatus.FORBIDDEN);
    ...
    ResponseEntity<ManagingResponse> removeHotelFromManager(@RequestHeader(value = HeaderConstant.AUTHORIZATION) String authorizationField,
        ...
        String userId = claims.getSubject();
        String role = claims.get(JwtUtil.ROLE, String.class);
        if (role.equals(MANAGER) && !userId.equals(managerId))
